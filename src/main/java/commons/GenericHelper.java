...
public class GenericHelper {
	...
	private void scrollDown(int durationForSwipe) {
		...
		new TouchAction((PerformsTouchActions) driver).press(PointOption.point(0, start))
	...
	public void scrollIntoView(String text) {
		if (DriverDetails.platform.equalsIgnoreCase("emulator") || DriverDetails.platform.equalsIgnoreCase("android")) {
			((AndroidDriver) driver).findElementByAndroidUIAutomator("new UiScrollable(new UiSelector()."
					+ "scrollable(true).instance(0)).scrollIntoView(new UiSelector()." + "textContains(\"" + text
					+ "\").instance(0))");
		} else {
	...
	private void swipeRight(int durationForSwipe) {
		...
		touchAction.press(PointOption.point(start_x, y))
				.waitAction(WaitOptions.waitOptions(Duration.ofSeconds(durationForSwipe)))
	...
	public WebElement scrollToElementWithText(String text) {
		WebElement element = ((FindsByAndroidUIAutomator) driver).findElementByAndroidUIAutomator(
